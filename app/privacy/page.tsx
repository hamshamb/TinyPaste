  ...
  return (
    <article className="mx-auto w-full max-w-[700px] px-4 py-12 sm:px-6 sm:py-16">
      ...
      <Section title="Encrypted pastes">
        <p>
          Turning on <strong className="font-medium text-text-base">Encrypt</strong> encrypts the text with
          AES-GCM using a random 256-bit key generated on your device. Only the ciphertext, the initialisation
          vector and a format version are sent to the server.
        </p>
        <p>
          The key is placed in the URL fragment — the part after{' '}
          <code className="rounded bg-surface-muted px-1 py-0.5 font-mono text-[13px]">#</code>. Browsers do
          not send fragments in requests, so the key never reaches {site.name}, is never written to a log, and
          is not stored by us. It is also never put in a query string or sent to any third party. If the
      ...
      <Section title="Expiry and deletion">
        <p>
          Once a paste&rsquo;s expiry time passes, every read path refuses it: the paste page, the raw route,
          the download route and the API. A cleanup job then removes expired rows from the database, so the
          data does not linger at rest.
        </p>
        <p>
